...
std::string CtaEngine::sendOrder(const std::string &vtSymbol, char order_type, double price, int volume, std::shared_ptr<StrategyBase> stg)
{
    ...
    else if (OrderType::CTAORDER_SELL == order_type)
    {
        ...
        // 上期所区分平今平昨
        if (contract.exchangeId.compare("SHFE"))   //QString::compare()
        {
...
{
    QuoteInfo* quote = (QuoteInfo*)ev.data_.get();     //data_ 是 shared_ptr指针,get返回存储的指针
...
{
    // 创建策略实例
    std::shared_ptr<StrategyBase> AtrRsi = std::move(std::shared_ptr<StrategyBase>(new StrategyAtrRsi(this, "AtrRsi", "ag1905")));
    // 获取策略类
    strategyDict["AtrRsi"] = AtrRsi;
    // 保存Tick映射关系
    tickStrategyDict["ag1905"].push_back(AtrRsi);
    // 订阅合约
    me->me_subscribe("ag1905");
}
